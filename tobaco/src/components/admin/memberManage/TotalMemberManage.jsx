...
class TotalMemberManage extends Component{
    ...
    async handleChange(thisState,userRow,index) {
        ...
        if(state) {
            ...
            const response = await axios.put(`http://localhost:8080/users/userEnDisable`, userNoList)
    ...
    async componentWillMount(){
        const response = await axios.get('http://localhost:8080/users/totalMembermanage/1')
        this.setState({
    ...
    async handleSearch() {
        ...
        const response = await axios.get(`http://localhost:8080/users/userSearch/total/${searchOption}/${userSearch}/1`)
        if(response.status ===200) {
            this.setState({
                userListResponse:response.data.userList,
                pagingResponse:response.data.paging
    ...
    async handleDisable() {
        ...
        userNoList.push("disable")
        const response = await axios.put(`http://localhost:8080/users/userEnDisable`, userNoList)
        if(response.status ===200) {
            alert("활동정지 되었습니다.")
            this.props.history.push('/DisableMemberManage')
        } else {
            alert("실패하였습니다.")
        }
    }
    ...
    async pagingPrev() {
        ...
        if(userSearch===""){
            const response = await axios.get(`http://localhost:8080/users/totalMembermanage/${page}`)
            this.setState({
                userListResponse:response.data.userList,
                pagingResponse:response.data.paging
            })
            ...
            const response = await axios.get(`http://localhost:8080/users/userSearch/total/${searchOption}/${userSearch}/${page}`)
            if(response.status ===200) {
                this.setState({
                    userListResponse:response.data.userList,
                    pagingResponse:response.data.paging
                        ...
                        <div>
                            ...
                            <div className="adminCol-md-4">
                                <div className="adminInput-group">
                                    <input type="text" id="userSearch" className="adminForm-control" placeholder="" onKeyUp={this.handleEnterKey.bind(this)}/>
                        ...
                        <div className="adminTable adminText-center">
                            <div className="adminCheckbox">
                                <label className="adminCol-md-12 adminThead">
                                    ...
                                    <div className="adminCol-md-2">닉네임</div>
                                    <div className="adminCol-md-3">가입일</div>
                                    <div className="adminCol-md-1">게시글 수</div>
